...
final String ERROR_STAGE = "initialize.directories"
...
def setFailure = { flowFile, String message, String details = null ->
  ...
  out = session.putAttribute(out, "error.message", message ?: "Directory initialization failed")
...
if (!missing.isEmpty()) {
  ...
  ff = session.putAttribute(ff, "setup.error", msg)
...
try {
  ...
  ff = setFailure(ff, e.message ?: "Directory initialization failed", e.toString())
  session.transfer(ff, REL_FAILURE)
}
